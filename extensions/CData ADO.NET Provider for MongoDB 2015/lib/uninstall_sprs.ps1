...
if (!$SQLServerVersion.StartsWith("11")) {
	Write-Error "Not necessary to execute this for your SQL Server." 
	exit
}
...
[System.IO.File]::Delete($sprsBinPath + $filename)
...
if ($ourCodeGroupNode -ne $null) {
	$CodeGroupNode.RemoveChild($ourCodeGroupNode) | Out-Null
}
...
if ($app) {
	Write-Output "Removing CData.MongoDB Data Extension..."
	$app | ForEach-Object {
		$tmp = Get-SPRSExtension -Identity $_.id -Name $entensionName -ExtensionType "Data"
		if($tmp) {
			Remove-SPRSExtension -Identity $_.id -Name $entensionName -ExtensionType "Data"
		}
	}
	Write-Output "CData.MongoDB Data Extension removed successfully."
} else {
	Write-Error "No SPRS Service Application."
}
Write-Output "Uninstall SPRS successfully."